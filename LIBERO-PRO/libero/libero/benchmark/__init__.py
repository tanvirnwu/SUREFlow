...
class Benchmark(abc.ABC):
    ...
    def get_task_bddl_file_path(self, i):
        ...
        # Some LIBERO-PRO suites reuse base-suite BDDL files and may not ship
        # dedicated bddl_files/<pro_suite>/... folders.
        if not os.path.exists(bddl_file_path):
            candidate_folder = self._fallback_problem_folder(self.tasks[i].problem_folder)
    ...
    def get_task_init_states(self, i):
        ...
        # Some LIBERO-PRO suites (e.g. libero_object_object / _swap / _task)
        # reuse the base suite initial states and may not provide a dedicated
        # folder under init_files/. Fall back to the base suite folder.
        if not os.path.exists(init_states_path):
            ...
            if candidate_folder is not None:
                ...
                if os.path.exists(candidate_path):
                    init_states_path = candidate_path

        if not os.path.exists(init_states_path):
            raise FileNotFoundError(
                f"Init states file not found for benchmark '{self.name}' task '{self.tasks[i].name}'. "
                f"Looked for: {init_states_path}"
            )
