...
from .eagle2_hg_model.inference_eagle_repo import (
...
class EagleBackbone(nn.Module):
    def __init__(
        ...
        processor = EagleProcessor(
            model_path=processor_cfg["model_path"] if processor_cfg else DEFAULT_EAGLE_MODEL_NAME,
            max_input_tiles=processor_cfg["max_input_tiles"] if processor_cfg else 1,
            model_spec=ModelSpecificValues(**processor_cfg["model_spec"]) if processor_cfg else None,
    ...
    def forward(self, vl_input: BatchFeature) -> BatchFeature:
        ...
        # Align dtype to avoid BF16/FP32 mismatch before the projection
        target_dtype = getattr(self.linear, "weight", None).dtype if hasattr(self.linear, "weight") else embeddings.dtype
